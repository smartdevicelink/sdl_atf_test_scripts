...
--  if GetInteriorVehicleDataConsent RPC request contains partially inconsistent values
--  in moduleType and moduleIds array (some of moduleIds belong to another moduleType)
--
-- Precondition:
...
local function buildTestModulesStruct(pRcCapabilities)
  ...
  for moduleType, consentArray in pairs(modulesStuct) do
    confusedModulesStruct[moduleType] = { }
    local validModuleId, validConsent = next(consentArray)
    local invalidModuleId, invalidConsent = next(modulesStuct[moduleConfuseMap[moduleType]])
    confusedModulesStruct[moduleType] = {
      [validModuleId] = validConsent,
      [invalidModuleId] = invalidConsent
    }
  end
