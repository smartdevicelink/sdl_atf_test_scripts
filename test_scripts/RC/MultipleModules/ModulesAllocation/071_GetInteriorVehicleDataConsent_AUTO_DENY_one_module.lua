...
-- Description:
--  Check of GetInteriorVehicleDataConsent RPC processing with one moduleId and module allocation is rejected
--  without asking driver if access mode is AUTO_DENY
--
-- Precondition:
...
--   Check:
--    SDL does not send GetInteriorVehicleDataConsent RPC to HMI
--    SDL rejects allocation of module (moduleType: <moduleType>, moduleId: <moduleId>) to App1
...
local rcCapabilities = common.initHmiRcCapabilitiesForRelease(testServiceArea)
...
for _, testModule in ipairs(testModulesArray) do
  runner.Step("Disallow module [" .. testModule.moduleType .. ":" .. testModule.moduleId
    ...
    { 1, testModule.moduleType, { [testModule.moduleId] = false }, rcAppIds, accessMode })
  runner.Step("Reject reallocation of module [".. testModule.moduleType .. ":" .. testModule.moduleId .. "] to App1",
    common.rejectedAllocationOfModuleWithoutConsent,
    { 1, testModule.moduleType, testModule.moduleId, nil, rcAppIds, "IN_USE" })
end
