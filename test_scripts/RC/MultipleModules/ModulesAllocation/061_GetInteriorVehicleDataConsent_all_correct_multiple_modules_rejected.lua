...
--  is rejected without asking driver if driver disallowed modules reallocation
--
...
--    HMI responds on GetInteriorVehicleDataConsent request with allowed: false for modules
--     (moduleType: <moduleType>, allowed: [false, false])
--   Check:
  ...
  for moduleType, modules in pairs(pRcCapabilities) do
    if not common.isTableContains(excludedModuleTypes, moduleType) then
      ...
      for _, rcModuleCapabilities in ipairs(modules) do
        modulesStuct[moduleType][rcModuleCapabilities.moduleInfo.moduleId] = false
      end
    end
  end
  return modulesStuct
