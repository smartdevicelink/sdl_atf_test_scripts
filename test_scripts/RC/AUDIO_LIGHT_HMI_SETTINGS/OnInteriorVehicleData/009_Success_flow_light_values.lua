...
--[[ Local Functions ]]
local function isSubscribed(pLightName)
  local mobSession = common.getMobileSession()
  local rpc = "OnInteriorVehicleData"

  local notificationHMIParams = common.getHMIResponseParams(rpc, Module)
  notificationHMIParams.moduleData.lightControlData.lightState[1].id = pLightName

  local notificationParams = common.getAppResponseParams(rpc, Module)
  notificationParams.moduleData.lightControlData.lightState[1].id = pLightName
...
for _, lightName in pairs(common.LightsNameList) do
  runner.Step("Send notification OnInteriorVehicleData Light name " .. lightName, isSubscribed, { lightName })
end
