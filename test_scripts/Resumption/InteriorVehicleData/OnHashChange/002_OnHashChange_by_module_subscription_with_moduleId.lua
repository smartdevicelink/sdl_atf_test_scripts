...
--
-- Sequence:
-- 1. GetInteriorVehicleData(subscribe = true, moduleType_1, moduleId) is requested
-- SDL does:
-- - a. send RC.GetInteriorVehicleData(subscribe = true, moduleType_1, moduleId) request to HMI
-- 2. HMI sends successful RC.GetInteriorVehicleData(moduleType_1, moduleId, isSubscribed = true) response to SDL
-- SDL does:
...
for _, moduleName in pairs(common.modules)do
  common.Step("OnHashChange after adding subscription to " .. moduleName, common.GetInteriorVehicleData,
    { moduleName, common.getModuleId(moduleName, 2), common.IVDataSubscribeAction.subscribe,
