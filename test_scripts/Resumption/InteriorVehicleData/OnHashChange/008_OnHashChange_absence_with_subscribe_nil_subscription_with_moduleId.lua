...
--  without subscribe parameter with specified moduleId
--
-- Precondition:
...
--
-- Sequence:
-- 1. GetInteriorVehicleData(moduleType_1, moduleId) is requested (without subscribe parameter)
-- SDL does:
-- - a. send RC.GetInteriorVehicleData(moduleType_1, moduleId) request to HMI
-- 2. HMI sends successful RC.GetInteriorVehicleData(moduleType, moduleId) response to SDL
-- SDL does:
...
for _, moduleName in pairs(common.modules)do
  ...
  common.Step("Absence OnHashChange after GetInteriorVD without subscribe to " .. moduleName,
    common.GetInteriorVehicleData, { moduleName, moduleId, withoutSubscribe, isNotCached, notExpectNotif })
  ...
  common.Step("Absence OnHashChange after GetInteriorVD without subscribe to " .. moduleName,
    common.GetInteriorVehicleData, { moduleName, moduleId, withoutSubscribe, isCached, notExpectNotif })
end
