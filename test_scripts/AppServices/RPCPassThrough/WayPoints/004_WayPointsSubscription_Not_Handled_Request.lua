...
--  4) app1 is activated and sends a PublishAppService request (with {serviceType=NAVIGATION, handledRPCs omitted} in the manifest)
--
--  SubscribeWayPoints
--
--  Steps:
...
--
--  Steps:
--  1) app2 sends a UnsubscribeWayPoints request to Core, HMI receives Navigation.UnsubscribeWayPoints request 
--  and responds with SUCCESS
--  2) app1 sends OnWayPointChange notification to Core
--  3) HMI sends Navigation.OnWayPointChange notification to Core
--
...
local function UnsubscribeWayPoints()
  ...
  common.getHMIConnection():ExpectRequest(unsubscribeRequest.hmi_name, unsubscribeRequest.hmi_params):Do(function(_, data)
      common.getHMIConnection():SendResponse(data.id, data.method, "SUCCESS", {})
  end)
...
runner.Title("Test")
runner.Step("SubscribeWayPoints HMI Unsupported SUCCESS", SubscribeWayPoints)
