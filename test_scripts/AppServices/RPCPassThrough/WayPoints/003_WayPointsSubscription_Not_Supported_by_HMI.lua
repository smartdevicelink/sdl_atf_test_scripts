...
--
--  Steps:
--  1) app2 sends a SubscribeWayPoints request to Core, HMI receives Navigation.SubscribeWayPoints request and responds with UNSUPPORTED_REQUEST
--  2) app1 sends OnWayPointChange notification to Core
--  3) HMI sends Navigation.OnWayPointChange notification to Core
--
--  Expected:
--  1) Core responds to SubscribeWayPoints with WARNINGS
--  2) Core forwards OnWayPointChange from app1 to app2
--  3) Core does not forward OnWayPointChange from HMI to app2
--
--  UnsubscribeWayPoints
--
--  Steps:
--  1) app2 sends a UnsubscribeWayPoints request to Core, HMI does not receive Navigation.UnsubscribeWayPoints request
--  2) app1 sends OnWayPointChange notification to Core
--  3) HMI sends Navigation.OnWayPointChange notification to Core
--
--  Expected:
...
local function SubscribeWayPoints()
  ...
  common.getHMIConnection():ExpectRequest(subscribeRequest.hmi_name, subscribeRequest.hmi_params):Do(function(_, data)
      common.getHMIConnection():SendResponse(data.id, data.method, "UNSUPPORTED_REQUEST", {})
  end)
...
local function UnsubscribeWayPoints()
  ...
  local cid = mobileSession:SendRPC(unsubscribeRequest.name, unsubscribeRequest.params)

  -- App will NOT handle the RPC (but the subscription will still apply to the app service)
  providerMobileSession:ExpectRequest(unsubscribeRequest.name, unsubscribeRequest.params):Times(0)
 
  -- HMI will not receive request since Core wasn't subscribe to HMI waypoints
  common.getHMIConnection():ExpectRequest(unsubscribeRequest.hmi_name, unsubscribeRequest.hmi_params):Times(0)

  mobileSession:ExpectResponse(cid, unsubscribeResponse.params)
end
...
runner.Title("Test")
runner.Step("SubscribeWayPoints HMI Unsupported WARNINGS", SubscribeWayPoints)
...
runner.Step("UnsubscribeWayPoints SUCCESS", UnsubscribeWayPoints)
