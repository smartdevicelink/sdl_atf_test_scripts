...
--  3) Application 1 sends UnregisterAppInterface
--
--  Expected:
--  1) SDL responds with GetSystemCapability SUCCESS
--  2) SDL send OnSystemCapabilityUpdated:REMOVED after app1 unregisters
---------------------------------------------------------------------------------------------------
...
local function UnregisterAppExpectNotification()
  ...
  mobileSession:ExpectResponse(cid, unregister_response)
...
runner.Step("Disconnect app service and expect OnSystemCapabilityUpdate", UnregisterAppExpectNotification)
