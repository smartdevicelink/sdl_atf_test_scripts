...
--
-- Description: hmi_capabilities_cache.json file doesn't contain all mandatory capability. Check that SDL use default
-- capability in case HMI doesn't send response for request from SDL.
--
-- Preconditions:
...
-- - b) not persist those capabilities in cache
---------------------------------------------------------------------------------------------------
...
local function getHMIParams(pMod, pReq)
  ...
  tbl[pMod][pReq].GetSystemInfo = nil
  return tbl
end
