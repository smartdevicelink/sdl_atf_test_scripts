---------------------------------------------------------------------------------------------------
-- Case: Send SendInternalError query with INTERNAL error code when establishing a secure audio
--       service, verify Core handles the message internally and that it does not interrupt the 
--       handshake process
---------------------------------------------------------------------------------------------------
...
local function startHandshake()
  ...
  local params = {
    ...
    payload = json.encode({
      id = constants.QUERY_ERROR_CODE.INTERNAL,
