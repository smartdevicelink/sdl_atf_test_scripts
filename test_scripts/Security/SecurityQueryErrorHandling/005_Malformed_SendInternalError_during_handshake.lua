---------------------------------------------------------------------------------------------------
-- Case: Send SendInternalError query with INTERNAL error code and a malformed payload when 
--       establishing a secure audio service, verify Core handles the message internally and that 
--       it does not interrupt the handshake process
---------------------------------------------------------------------------------------------------
...
local function startHandshake()
  ...
  local params = {
    ...
    payload = json.encode(constants.QUERY_ERROR_CODE.INTERNAL),
    binaryData = string.char(constants.QUERY_ERROR_CODE.INTERNAL)
  }
