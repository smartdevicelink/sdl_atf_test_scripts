...
-- [Mobile Navi] SDL must respond NACK in case navigation app connected over protocol v2 sends StartService for audio service
--
-- Description:
-- In case
...
-- SDL must:
-- start TLSHandshake sequence (by sending TLS Hello to this mobile app)
--
-- 1. Used preconditions:
-- Navi app exists in LP, certificate is in module_config
--
-- 2. Performed steps
-- 2.1. Start session using protocol version = 2, register and activate application
-- 2.2. Start secure Audio service
--
-- Expected result:
-- 1. Application is registered and activated successfully
-- 2. SDL doesn't trigger PTU
-- 3. SDL returns StartServiceNACK to Audio
---------------------------------------------------------------------------------------------
...
function Test:Verification()
  ...
  EXPECT_HMICALL("BasicCommunication.PolicyUpdate")
  :Do(function(e, _) print("BC.PolicyUpdate   ", e.occurences) end)
  :Times(0)
