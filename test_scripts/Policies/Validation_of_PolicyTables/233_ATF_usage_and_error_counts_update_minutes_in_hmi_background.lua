---------------------------------------------------------------------------------------------
-- Requirement summary:
-- [Policies] "usage_and_error_counts" and "minutes_in_hmi_background" update
--
-- Check update value of "minutes_in_hmi_background" in Local Policy Table.
-- 1. Used preconditions:
-- Start default SDL
-- Add MobileApp to PreloadedPT
-- InitHMI register MobileApp in BACKGROUND
--
-- 2. Performed steps:
-- Wait <N> minutes
-- Activate MobileApp BACKGROUND -> FULL
-- Wait <M> minutes
-- Stop SDL
...
function Test.AppInBackgroundNMinutes()
  local delta = 8
...
function Test.AppInBackgroundXMinutes()
  local delta = 6
  ...
  local checks = {
    {
      query = table.concat(
        {
          'select minutes_in_hmi_background from app_level where application_id = "',
      ...
      expectedValues = {table.concat(
          {
            "",
            TESTED_DATA.expected.policy_table.usage_and_error_counts.app_level[config.application1.registerAppInterfaceParams.fullAppID].minutes_in_hmi_background
          })
      }
    }
  }
