...
-- 6) App sends ResetGlobalProperties with properties: {"VRHELPTITLE"}
-- SDL does:
--  a. Send UI.SetGlobalProperties to the HMI with vrHelp and vrHelpTitle
-- 7) App sends ResetGlobalProperties with properties: {"VRHELPITEMS"}
-- SDL does:
--  a. Send UI.SetGlobalProperties to the HMI with vrHelp and vrHelpTitle
-- 8) App sends ResetGlobalProperties with properties: {"HELPPROMPT"}
-- SDL does:
--  a. Send TTS.SetGlobalProperties to the HMI with helpPrompt
--
-- Steps:
...
runner.Step("ResetGlobalProperties VRHELPTITLE", common.ResetGlobalProperties, {{ properties = {"VRHELPTITLE"} }})
runner.Step("ResetGlobalProperties VRHELPITEMS", common.ResetGlobalProperties, {{ properties = {"VRHELPITEMS"} }})
runner.Step("ResetGlobalProperties HELPPROMPT", common.ResetGlobalProperties, {{ properties = {"HELPPROMPT"} }})
