...
---------------------------------------------------------------------------------------------------
-- Description: SDL sends AddCommand response with UNSUPPORTED_RESOURCE, success:false for single UI-related RPC
...
-- Steps:
-- 1. App requests AddCommand only with UI part to SDL
-- SDL does:
-- - not send UI.AddCommand request to HMI
-- - respond AddCommand(resultCode: UNSUPPORTED_RESOURCE, success: false) to App
---------------------------------------------------------------------------------------------------
...
local rpcParams = {
  rpc = "AddCommand",
  requestParam = { cmdID = 11, menuParams = { position = 0, menuName ="menuName_11" }},
...
common.Title("Test")
common.Step("App sends AddCommand", common.sendRPC, { rpcParams })
