---------------------------------------------------------------------------------------------------
-- User story: https://github.com/smartdevicelink/sdl_core/issues/3014
---------------------------------------------------------------------------------------------------
-- Description: SDL responds with GetVehicleData(success = true, resultCode = "VEHICLE_DATA_NOT_AVAILABLE")
--  in case HMI responds with error struct
--
-- Precondition:
...
-- 2. HMI responds with `DATA_NOT_AVAILABLE` result code in error struct
-- SDL does:
...
local testData = {
  ...
  respFunc = function(data)
    common.getHMIConnection():SendError(data.id, data.method, "DATA_NOT_AVAILABLE", "Error message") end,
...
common.Title("Test")
common.Step("GetVehicleData", common.getVehicleData, { testData })
