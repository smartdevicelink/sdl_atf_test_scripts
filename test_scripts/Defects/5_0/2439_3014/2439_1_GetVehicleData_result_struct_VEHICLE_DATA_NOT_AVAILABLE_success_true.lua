...
--  in case HMI responds with result struct and some part of requested data
--
-- Precondition:
...
-- 2. HMI responds with `DATA_NOT_AVAILABLE` result code and speed data in result struct
-- SDL does:
--  - sends GetVehicleData(success = true, resultCode = "VEHICLE_DATA_NOT_AVAILABLE") to mobile app
---------------------------------------------------------------------------------------------------
...
local testData = {
  ...
  respFunc = function(data)
    common.getHMIConnection():SendResponse(data.id, data.method, "DATA_NOT_AVAILABLE", { speed = 50 }) end,
  respExp = { success = true, resultCode = "VEHICLE_DATA_NOT_AVAILABLE", speed = 50 }
}
