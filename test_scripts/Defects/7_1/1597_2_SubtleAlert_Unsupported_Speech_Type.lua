...
--
-- Steps: 
-- 1) Send SubtleAlert mobile RPC from app with ttsChunks, HMI responds with UNSUPPORTED_RESOURCE to 
--    Speak request
--
-- Expected:
-- 1) App receives SubtleAlert response with info from Speak response, WARNINGS result code, 
--    and success=true
---------------------------------------------------------------------------------------------------
...
local function sendSubtleAlert()
  local cid = common.getMobileSession():SendRPC("SubtleAlert", { 
    alertText1 = "Message", 
    ttsChunks = { 
...
runner.Title("Test")
runner.Step("Subtle Alert", sendSubtleAlert)
