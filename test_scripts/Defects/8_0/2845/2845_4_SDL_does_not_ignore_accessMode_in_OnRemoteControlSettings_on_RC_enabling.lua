...
--  (checking of "accessMode" changes when RC is in enabling process)
--
-- In case:
...
-- 4. RC functionality is enabled from HMI via OnRemoteControlSettings with setting of <mode> access mode
--  OnRemoteControlSettings [allowed = true, accessMode = <mode>]
--  where <mode> is one of {"AUTO_DENY", "ASK_DRIVER", "AUTO_ALLOW"}
--
-- SDL does:
--  - apply received <mode> accessMode and enabling of RC functionality
...
for _, mod in pairs(common.modules)  do
  for _, rpc in pairs(rcRpcs) do
    if common.isRpcApplicable(mod, rpc) then
      runner.Title("Changing of the access mode on RC enabling; Module: " .. mod .. "; RPC: " .. rpc)
      for _, check in ipairs(checks) do
        ...
        runner.Step("Enable RC from HMI and set " .. check.to .. " access mode", common.defineRAMode, { on, check.to })
        runner.Step("Activate App1", common.activateApp, { apps[1] })
