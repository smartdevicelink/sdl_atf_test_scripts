...
--
-- Description: SDL initiates the OEM mapping update in case VDI were updated, with an ignition cycle inbetween

-- Precondition:
-- 1. Preloaded file contains VehicleDataItems for all RPC spec VD
-- 2. App is registered
-- 3. PTU is triggered
-- 4. PTU is performed with updated VDI and OEM mapping version
-- 5. Ignition cycle is performed, App is re-registered and activated


-- Sequence:
-- 1. HMI requests SDL.GetPolicyConfigurationData(policyType = "module_config", property = "endpoint_properties")
--  a. SDL responds to SDL.GetPolicyConfigurationData with endpoint_properties from DB
-- 2. PTU is performed with updated VDI and OEM mapping version
-- 3. HMI sends SDL.GetPolicyConfigurationData(policyType = "module_config", property = "endpoint_properties")
--  a. SDL responds SDL.GetPolicyConfigurationData with endpoint_properties from DB
-- 4. HMI sends SDL.GetPolicyConfigurationData(policyType = "module_config", property = "endpoint")
--  a.  SDL responds to SDL.GetPolicyConfigurationData with endpoint from DB
-- 5. HMI sends BC.OnSystemRequest(requestType = "OEM_SPECIFIC", requestSubType = "VEHICLE_DATA_MAPPING",
--   fileType = "JSON", url) to SDL
--  a. SDL resend BC.OnSystemRequest to mobile app
-- 6. Mobile app sends SystemRequest(requestType = "OEM_SPECIFIC", requestSubType = "VEHICLE_DATA_MAPPING") to
...
-- 7. HMI responds with success resultCode to BC.SystemRequest
