...
-- Item: Happy path (DEFAULT_ACTION softbutton pressed)
--
-- Requirement summary:
-- [SubtleAlert] SUCCESS: request with UI portion including DEFAULT_ACTION softbutton and TTSChunks 
-- in background
--
-- Description:
...
-- d. appID1 has policy permissions to steal focus and send SubtleAlert in background
...
-- User selects DEFAULT_ACTION button in SubtleAlert prompt
...
-- SDL receives Buttons.OnButtonEvent/OnButtonPress notification for DEFAULT_ACTION softbutton
...
local function subtleAlertTwoApps(pParams)
  ...
  :Do(function(_, data)
      ...
      local function alertResponseDefaultAction()
        local buttonID = requestParams.softButtons[1].softButtonID
      ...
      common.runAfter(alertResponseDefaultAction, 3000)
    end)
...
runner.Title("Test")
runner.Step("SubtleAlert with two apps DEFAULT_ACTION Positive Case", subtleAlertTwoApps, { allParams })
