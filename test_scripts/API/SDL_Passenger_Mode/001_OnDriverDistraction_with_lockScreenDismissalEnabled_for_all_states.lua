...
-- 3) HMI sends "lockScreenDismissalEnabled" item (boolean) (with all mandatories) as a parameter of OnDriverDistraction notification
-- 4) App activated (HMI level FULL)
-- 5) HMI sends "lockScreenDismissalEnabled" item (boolean) (with all mandatories) as a parameter of OnDriverDistraction notification
-- 6) Mobile app received OnHMIStatus (hmiLevel = "LIMITED")
-- 7) HMI sends "lockScreenDismissalEnabled" item (boolean) (with all mandatories) as a parameter of OnDriverDistraction notification
-- 8) Mobile app received OnHMIStatus (hmiLevel = "BACKGROUND")
-- 9) HMI sends "lockScreenDismissalEnabled" item (boolean) (with all mandatories) as a parameter of OnDriverDistraction notification
-- SDL does:
...
for _, v in ipairs(common.OnDDValue) do
    for _, k in ipairs(common.value) do
        runner.Step("OnDriverDistraction with state " .. v .. " with lockScreenDismissalEnabled " .. tostring(k) , common.onDriverDistraction, { v, k })
    end
end
