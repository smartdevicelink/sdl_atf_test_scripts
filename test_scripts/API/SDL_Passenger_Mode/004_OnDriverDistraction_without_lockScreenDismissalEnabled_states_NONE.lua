...
-- 3) HMI not sends "lockScreenDismissalEnabled" item (with all mandatories) as a parameter of OnDriverDistraction notification
...
-- 2) Send  OnDriverDistraction notification to mobile without "lockScreenDismissalEnabled" parameter when (HMI level FULL)
---------------------------------------------------------------------------------------------------
...
for _, v in pairs(common.OnDDValue) do
  runner.Step("HMI sends OnDriverDistraction without lockScreenDismissalEnabled", common.onDriverDistractionUnsuccess, { v, nil })
end
