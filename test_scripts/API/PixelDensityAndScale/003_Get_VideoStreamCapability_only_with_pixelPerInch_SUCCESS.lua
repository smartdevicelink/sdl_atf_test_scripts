...
-- Description:
-- In case:
-- 1) During start HMI provides VideoStreamingCapability for parameter: "pixelPerInch = 1000.11"
-- 2) Mob app sends GetSystemCapability request to SDL
-- SDL does:
-- 1) send response to Mobile with videoStreamingCapability all mandatory parameters and "pixelPerInch = 1000.11"
--    got from HMI
---------------------------------------------------------------------------------------------------
...
runner.Title("Test")
runner.Step("Get Capability", common.getSystemCapability, { nil, pixelPerInch, nil })
