...
-- 3) HMI sends the `FuelRange` structure with only one param with boundary value in GetVehicleData response
-- SDL does:
-- 1) respond with resultCode:"SUCCESS" to app with only one param with boundary value
---------------------------------------------------------------------------------------------------
...
for parameterName in pairs(common.allVehicleData) do
  for _, value in pairs (common.allVehicleDataBoundaryValues[parameterName]) do
    common.Step("HMI sends response with " .. parameterName .. "=" .. value,
      common.getVehicleData, { { { [parameterName] = value } } })
  end
end
