...
--
-- Description: Check subscription on StabilityControlsStatus data with valid HMI response
--   and other parameters with invalid HMI response via SubscribeVehicleData RPC
--
-- Precondition:
...
--   - gps is an empty structure
--   - gps contains invalid values
--   - gps is of an incorrect type
...
-- SDL sends VehicleInfo.SubscribeVehicleData request with (stabilityControlsStatus = true,
--   gps = true) to HMI
-- HMI sends invalid VehicleInfo.SubscribeVehicleData response and gps is absent
-- SDL sends SubscribeVehicleData response with (success: false, resultCode: "GENERIC_ERROR",
