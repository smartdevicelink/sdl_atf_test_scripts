...
--  in SubscribeVehicleData request.
--
-- In case:
-- 1) App sends invalid SubscribeVehicleData request to the SDL.
-- SDL does:
--  a) not transfer this request to HMI.
--  b) send response SubscribeVehicleData(success:false, resultCode:`INVALID_DATA`) to mobile app.
---------------------------------------------------------------------------------------------------
...
common.Title("Test")
common.Step("SubscribeVehicleData request with invalid data type of gearStatus parameter", common.processRPCFailure,
