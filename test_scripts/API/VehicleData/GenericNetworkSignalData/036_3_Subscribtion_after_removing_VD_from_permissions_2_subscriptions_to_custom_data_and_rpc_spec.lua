...
--
-- Description: Processing VD subscription in case when app is subscribed to custom data_1 and RPC spec data_2
...
-- 7. App is subscribed to RPC spec data_2
...
-- 4. Mobile app1 requests SubscribeVehicleData(data_1)
...
--   a. SDL resends the OnVD notification to mobile app
-- 10. Mobile app1 requests SubscribeVehicleData(data_1)
--   a. SDL rejects requests with INVALID_DATA resultCode
---------------------------------------------------------------------------------------------------
...
local function registerAppWithResumption()
  local expectedRequest = { [rpcSpecData] = true }
  ...
  :Do(function(_, data)
    common.getHMIConnection():SendResponse(data.id, data.method, "SUCCESS", {[rpcSpecData] = {
      dataType = common.VehicleDataItemsWithData[rpcSpecData].APItype,
      resultCode = "SUCCESS"
    }})
  end)
  :ValidIf(function(_, data)
...
runner.Step("SubscribeVehicleData " .. rpcSpecData, common.VDsubscription,
  { appSessionId, rpcSpecData, "SubscribeVehicleData" })
runner.Step("OnVehicleData " .. rpcSpecData, common.onVD,
  { appSessionId, rpcSpecData })
