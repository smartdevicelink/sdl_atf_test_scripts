...
-- and app2 is subscribed for custom data_1 and RPC spec data_3  and after PTU is performed
...
-- 7. App2 is subscribed to custom data_2 and RPC spec data_3
...
--   a. SDL does not resend the OnVD notification to mobile app1 and app2
...
-- 5. Mobile app2 requests SubscribeVehicleData(data_1)
--   a. SDL rejects requests with INVALID_DATA resultCode
...
--   a. SDL rejects requests with INVALID_DATA resultCode
-- 13. Mobile app2 requests SubscribeVehicleData(data_1)
--   a. SDL rejects requests with INVALID_DATA resultCode
---------------------------------------------------------------------------------------------------
...
local function subscriptionWithoutHMIrequest()
  local cid = common.getMobileSession(appSessionId2):SendRPC("SubscribeVehicleData", { [itemToRemove] = true })
  common.getHMIConnection():ExpectRequest("VehicleInfo.SubscribeVehicleData")
  :Times(0)

  common.getMobileSession(appSessionId2):ExpectResponse(cid, {
    success = true,
    resultCode = "SUCCESS",
    [itemToRemove] = {
      dataType = common.CUSTOM_DATA_TYPE,
  ...
  common.getMobileSession(appSessionId2):ExpectNotification("OnHashChange")
  :Do(function(_,data)
...
runner.Step("App2 SubscribeVehicleData " .. itemToRemove, subscriptionWithoutHMIrequest)
...
runner.Step("App1 and App2 OnVehicleData " .. itemToRemove, common.onVD2Apps,
  { itemToRemove, common.VD.NOT_EXPECTED, common.VD.NOT_EXPECTED })
runner.Step("App1 OnVehicleData " .. customVDapp1, common.onVD,
  { appSessionId1, customVDapp1 })
runner.Step("App2 OnVehicleData " .. rpcSpecDataApp2, common.onVD,
  { appSessionId2, rpcSpecDataApp2 })
