...
--1.1. Request is sent with with invalid image parameter in audioPassThruIcon and with all mandatory parameters within bounds (samplingRate, maxDuration, bitsPerSample, audioType)
--2. Performed steps
-- Send PerformAudioPassThru (audioPassThruIcon with invalid image type, mandatory params) from mobile to SDL and check:
...
function Test:TestStep_Mandatory_Params_audioPassThruIcon_InvalidType_Image()
