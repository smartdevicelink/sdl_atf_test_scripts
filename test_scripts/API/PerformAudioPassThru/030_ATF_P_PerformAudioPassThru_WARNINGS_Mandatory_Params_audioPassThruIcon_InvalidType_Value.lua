...
--1.1. Request is sent with invalid value parameter in audioPassThruIcon (integer instead of string) and with all mandatory parameters within bounds (samplingRate, maxDuration, bitsPerSample, audioType)
--2. Performed steps
-- Send PerformAudioPassThru (audioPassThruIcon with value = 123, mandatory params) from mobile to SDL and check:
...
function Test:TestStep_Mandatory_Params_audioPassThruIcon_InvalidType_Value()
