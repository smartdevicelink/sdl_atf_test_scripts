...
function c.rpcs.SendLocation( pExpTimoutForMobResp, pExpTimeBetweenResp, pHMIRespFunc, pOnRTParams, pExpMobRespParams )
  ...
  :ValidIf(function()
    local checkResult, resultMessage = c.responseTimeCalculation(pExpTimeBetweenResp, pOnRTParams)
    return checkResult, resultMessage
  end)
end
...
--[[ @responseTimeCalculation: compare expected and actual time between starting event(OnResetTimeout notification from HMI or request from mobile app) and mobile response
--! @parameters:
--! pExpTimeBetweenResp - time between the mobile response and sending the OnResetTimeout notification from HMI or request from mobile app
--! pOnRTParams - parameters for BC.OnResetTimeout
