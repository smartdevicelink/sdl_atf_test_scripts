...
-- 3) HMI sends 'BC.OnResetTimeout' notification to SDL with 'resetPeriod=9s' parameter within the delay of 7s
-- after receiving request from SDL:
-- 4) HMI doesn't provide a response
-- SDL does:
--  - wait for the response from HMI within 'resetPeriod + delay + DefaultTimeoutCompensation' (17s)
...
local timeout = paramsForRespFunction.resetPeriod + common.defaultTimeoutCompensation
...
for _, rpc in pairs(common.rpcsArray) do
  common.Step("Send " .. rpc , common.rpcs[rpc],
    { timeout + paramsForRespFunction.notificationTime + 1000,
      timeout,
      common.onResetTimeoutOnly, paramsForRespFunction, rpcResponse, common.responseTimeCalculationFromNotif })
end
