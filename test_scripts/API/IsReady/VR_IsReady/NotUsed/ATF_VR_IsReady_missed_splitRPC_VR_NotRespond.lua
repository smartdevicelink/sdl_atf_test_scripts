...
-- Specific case: VR interface does not respond.
---------------------------------------------------------------------------------------------
...
-----------------------------------------------------------------------------------------------
	...
	local function checksplit_VR_RPCs_VR_does_not_Respond(TestCaseName, resultCodes)
		...
		for i = 1, #resultCodes do
			...
			Test[TestCaseName .. "_AddCommand_VR_does_not_respond_UI_responds_".. tostring(resultCodes[i].resultCode)] = function(self)
		...
		for i = 1, #resultCodes do
			...
			Test[TestCaseName .. "_DeleteCommand_VR_does_not_respond_UI_responds_".. tostring(resultCodes[i].resultCode)] = function(self)
				...
				EXPECT_HMICALL("UI.DeleteCommand", {cmdID = commandID})
				:Do(function(_,data)
		...
		for i = 1, 1 do					
			...
			Test[TestCaseName .. "_PerformInteraction_VR_does_not_respond_UI_responds_" .. tostring(resultCodes[i].resultCode)] = function(self)
				...
				:Do(function(_,data)
					...
					local function vrResponse()
						...
						--self.hmiConnection:SendResponse(data.id, data.method, resultCodes[i].resultCode, {choiceID = i})
		...
		for i = 1, #resultCodes do
			Test[TestCaseName .. "_ChangeRegistration_VR_does_not_respond_UI_responds_" .. resultCodes[i].resultCode] = function(self)
	...
	for i=1, #TestData do
		...
		checksplit_VR_RPCs_VR_does_not_Respond(TestCaseName, resultCodes)
