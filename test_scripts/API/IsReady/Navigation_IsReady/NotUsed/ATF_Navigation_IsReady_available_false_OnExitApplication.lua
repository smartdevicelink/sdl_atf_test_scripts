...
------------------------
--Begin of SpecialHMIResponse.1.2

--Functional Requirement: APPLINK-18509 OnExitApplication(UNSUPPORTED_HMI_RESOURCE, appID): SDL must unregister application with appID from notification
-- Description:
-- In case
-- SDL receives OnExitApplication (UNSUPPORTED_HMI_RESOURCE, <appID>) from HMI
-- SDL must:
-- unregister application with <appID> via OnAppInterfaceUnregistered(UNSUPPORTED_HMI_RESOURCE) notification
-- clear all internal information about app's with <appID> non-responded RPCs 
-- ignore all responses and notifications from HMI associated with <appID> if received from HMI
-- HMI expected behavior:
-- a. In case navigation interface is not supported AND navigation app registers -> HMI is expected:
-- a1. to ignore all requests from SDL associated with this <appID_of_navigation_app>
-- a2. to reject registration for this navigation application via OnExitApplication(UNSUPPORTED_HMI_RESOURCE, <appID>) notification

local function sequence_check_OnExitApplication_UNSUPPORTED_HMI_RESOURCE()
	Test[APIName .. "_Check_OnExitApplication_UNSUPPORTED_HMI_RESOURCE"] = function(self)
		local params = {
			syncMsgVersion =
			{
				majorVersion = 3,
		...
		local cidRAI = self.mobileSession:SendRPC("RegisterAppInterface", params)
		...
		--mobile side: expect response for BasicCommunication.OnAppRegistered
		self.mobileSession:ExpectResponse(cidRAI, { resultCode = "SUCCESS", success = true	})
		:Timeout(2000)	
		...
		:Times(2) --1 for Register and 1 for Unregister
...
sequence_check_OnExitApplication_UNSUPPORTED_HMI_RESOURCE()
--End of SpecialHMIResponse.1.2
