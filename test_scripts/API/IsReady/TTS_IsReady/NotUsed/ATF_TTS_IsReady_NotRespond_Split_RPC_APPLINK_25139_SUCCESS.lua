	...
	for i = 1, #TestData_Error do
		for j = 1, #TestData_SUCCESS_CODE do
			Test["AlertManeuver"..TestCaseName .. "Others_respond_"..TestData_SUCCESS_CODE[j].resultCode .."_TTS_responds_" .. TestData_Error[i].resultCode] = function(self)
				...
				--mobile side: expect response
				if(resultCode == "UNSUPPORTED_RESOURCE") then
		...
		for j = 1, #TestData_SUCCESS_CODE do
			Test["AlertManeuver"..TestCaseName .. "Others_respond_"..TestData_SUCCESS_CODE[j].resultCode .."_TTS_responds_" .. TestData_Error[i].resultCode] = function(self)
				...
				:Do(function(_,data)
					AlertId = data.id
					local function alertResponse()
						self.hmiConnection:SendResponse(AlertId, "Navigation.AlertManeuver", TestData_SUCCESS_CODE[j].resultCode, {})
					end
				...
				:Do(function(_,data)
						...
						local function speakResponse()
							self.hmiConnection:SendError(data.id, "TTS.Speak", TestData_Error[i].resultCode, "error message")
...
local TestData = {
	...
	-- {caseID = 24, description = "method_IsInvalidCharacter_Splace"},
	-- {caseID = 25, description = "method_IsInvalidCharacter_Tab"},
