...
--
-- 2. Performed steps:
-- a) HMI sends OnSeekMediaClockTimer notification to SDL with fake parameter.
--
-- Expected result:
-- a) SDL cuts fake parameter and resends OnSeekMediaClockTimer notification to mobile app.
---------------------------------------------------------------------------------------------
...
function Test:TestStep_OnSeekMediaClockTimer_With_Fake_Parameter()
 self.hmiConnection:SendNotification("UI.OnSeekMediaClockTimer",{fakeparameter = "fake", seekTime =  {hours = 1, minutes = 1, seconds = 59}, appID = self.applications["Test Application"]})

 EXPECT_NOTIFICATION("OnSeekMediaClockTimer", {seekTime = {hours = 1, minutes = 1, seconds = 59}})
  :ValidIf(function(_,data)
    if data.fakeparameter then
    print("Fake parameter present in notification!")
    return false
    end
    return true
  end)
